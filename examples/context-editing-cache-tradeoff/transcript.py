"""Builds the synthetic tool-heavy transcript this example clears and caches.

Lifted almost verbatim from `examples/context-editing-preview/transcript.py`
(same research lineage, same shape): only this docstring differs, and the copy
is deliberate - each example in this repo is a self-contained directory with its
own pin, so a cross-example import would couple two of them at the filesystem
level.

Pure and deterministic: the same `(rounds, result_chars)` always produces the
same messages, byte for byte. That matters twice over here - a transcript with
any per-run variation in it would be a fresh cache write every turn, and the
clearing boundary would move for reasons that have nothing to do with the edit.
...
Its cleared region is deep history, well below the rolling cache breakpoint,
which is the case the payback model in `payback.py` assumes.

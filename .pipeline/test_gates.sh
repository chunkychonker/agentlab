...
src_noise="$( { . "$REPO/.pipeline/verdict.sh"; . "$REPO/.pipeline/health.sh"; . "$REPO/.pipeline/pipeline_health.sh"; . "$REPO/.pipeline/backlog.sh"; . "$REPO/.pipeline/preflight.sh"; . "$REPO/.pipeline/postcondition.sh"; . "$REPO/.pipeline/schedule.sh"; } 2>&1 )"
src_rc=$?
assert_eq "INV1" "0|" "$src_rc|$src_noise" \
  "sourcing all seven libs exits 0 and prints nothing"
...
assert_eq "R5" "" "$syntax_bad" \
  "bash -n is clean on run.sh, the seven libs, and this test"

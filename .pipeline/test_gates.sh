...
src_noise="$( { . "$REPO/.pipeline/verdict.sh"; . "$REPO/.pipeline/health.sh"; . "$REPO/.pipeline/pipeline_health.sh"; . "$REPO/.pipeline/backlog.sh"; . "$REPO/.pipeline/preflight.sh"; . "$REPO/.pipeline/postcondition.sh"; . "$REPO/.pipeline/schedule.sh"; . "$REPO/.pipeline/network_retry.sh"; } 2>&1 )"
src_rc=$?
assert_eq "INV1" "0|" "$src_rc|$src_noise" \
  "sourcing all eight libs exits 0 and prints nothing"
...
syntax_bad="$(for f in "$RUN_SH" "$REPO/.pipeline/verdict.sh" "$REPO/.pipeline/health.sh" \
  ...
  "$REPO/.pipeline/schedule.sh" "$REPO/.pipeline/network_retry.sh" \
  "$REPO/.pipeline/test_gates.sh"; do
    bash -n "$f" 2>&1
  done)"
assert_eq "R5" "" "$syntax_bad" \
  "bash -n is clean on run.sh, the eight libs, and this test"
...
# Membership in the loop, not position in it: the list grows as libs are added
# (postcondition.sh landed between preflight and schedule), and a pattern that
# pins the neighbours fails on the next lib rather than on a real regression.
assert_eq "W25" "1" \
  "$(grep -c 'for lib in .* schedule[ ;]' "$RUN_SH")" \
